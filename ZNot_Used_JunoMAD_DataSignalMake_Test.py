...
data = Juno_Mag_MakeData_Function.read_24hData(year_doy_pj,freq=60)
# Time_start = data.index.min()+Juno_Mag_MakeData_Function.hour_1*6 + Juno_Mag_MakeData_Function.min_1*45
# Time_end = data.index.max()
# Time_end = Time_start+Juno_Mag_MakeData_Function.min_1*50

# data = data.loc[Time_start:Time_end]


B_Ex = Juno_Mag_MakeData_Function.MagneticField_External(data)
B_In = Juno_Mag_MakeData_Function.MagneticField_Internal(data, model='jrm33',degree=20)
...
B_residual = Juno_Mag_MakeData_Function.Caluclate_B_Residual(data,B_In,B_Ex)
...
for time, group in grouped_data:
    ...
    plt.subplot(4,2,1)
    plt.title(f'Residual B Field (nT) \n {time} - {time + Juno_Mag_MakeData_Function.min_1 * 25}')
    component = 'Bx'
